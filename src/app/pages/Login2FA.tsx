  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
        <p className="text-sm text-[var(--color-text-lighter-lm)]">
        ...
        <div className="flex gap-2 justify-center" onPaste={handlePaste}>
          {values.map((val, i) => (
            <input
              ...
              maxLength={1}
              className="w-12 h-12 text-center text-xl border rounded bg-[#FFFFFF]"
            />
          ))}
        </div>
        ...
        <button
          type="submit"
          className="w-20 px-0 py-2 rounded-[12px] font-medium bg-[#C23D00] text-[#FFFFFF]"
        >
          Confirm
        </button>
      </form>
