...
export function PostBody({ title, user, content, tags }: PostContent) {
  return (
    <div className="flex flex-col gap-3 bg-secondary-lm hover:bg-hover-lm transition border-2 border-stroke-grey hover:border-stroke-peach p-8 rounded-2xl">
      ...
      {/* Tags: appear under the title, side-by-side */}
      {tags && tags.length > 0 && (
      ...
      <div className="w-full h-120 overflow-hidden">
        {content.img && <img src={content.img} className="object-cover object-center w-full h-full" />}
      </div>
